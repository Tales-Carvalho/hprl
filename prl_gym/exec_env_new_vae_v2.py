    ...
    def _execute_gt_program(self, config, program_seq):
        ...
        if num_demo < config.num_demo_per_program and 'maze' not in config.env_task:
            if config.cover_all_branches_in_demos and exec_ratio is not None and exec_ratio <= 1.0:
                if self.config.env_task == 'program':
                    print("WARNING: could generate {} with only {}% coverage of GT program {}".format(num_demo, exec_ratio*100, gt_code))
                else:
        ...
        # s_h shoud NOT be empty
        if num_demo == 0:
            assert False, "No demo generated from program: {}".format(gt_code)
...
class ExecEnv2(ExecEnv):
    ...
    def __init__(self, config, metadata={}):
        ...
        print("ExecEnv2 on init, env_task: ", config.env_task)
        ...
        if init_states is None:
            if 'topOff' not in config.env_task:
                self.init_states = [self.init_func(config.input_height, config.input_width, config.wall_prob) for _ in range(config.num_demo_per_program)]
            else:
                self.init_states = [self.init_func(config.input_height, config.input_width, config.wall_prob, is_top_off=True) for _ in range(config.num_demo_per_program)]
        else:
    ...
    def reset(self):
        self.init_states = [self.init_func(self.config.input_height, self.config.input_width, self.config.wall_prob) for _ in
